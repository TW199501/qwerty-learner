...
export default function StartButton({ isLoading }: { isLoading: boolean }) {
  ...
  return (
    <Tooltip content={`${state.isTyping ? t('typing.pause') : t('typing.start')} (Enter)`} className="box-content h-7 w-8 px-6 py-1">
      <div
      ...
      >
        <button
          ...
          aria-label={state.isTyping ? t('typing.pause') : t('typing.start')}
        >
          <span className="font-medium">{state.isTyping ? t('typing.pause') : t('typing.start')}</span>
        </button>
        {isShowReStartButton && (
